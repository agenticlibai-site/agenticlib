...
const ynBtnStyle = (selected: boolean): React.CSSProperties => ({
  ...
  borderRadius: 8,
  cursor: "pointer",
  border: `1.5px solid ${selected ? GREEN : "rgba(0,0,0,0.12)"}`,
  background: selected ? "rgba(5,150,105,0.08)" : "#fff",
...
const radioOptStyle = (selected: boolean): React.CSSProperties => ({
  ...
  borderRadius: 8,
  cursor: "pointer",
  background: selected ? "rgba(5,150,105,0.06)" : "transparent",
  transition: "border-color 0.15s, background 0.15s",
});

// ── Component ─────────────────────────────────────────────────────────────────
export default function RalfiFeedbackPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/ralfi-feedback", {
        method: "POST",
            ...
            >
              ...
              <div style={qStyle}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <label htmlFor="company" style={{ ...labelStyle, fontSize: 13, fontWeight: 600, color: GREEN, letterSpacing: "0.04em", textTransform: "uppercase" as const }}>
                    Company
                  </label>
                  <input
                    id="company"
              ...
              <div style={qStyle}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    {[
                      ...
                      <label key={opt.val} style={radioOptStyle(q2 === opt.val)}>
                        <span style={{
                          ...
                          border: `2px solid ${q2 === opt.val ? GREEN : "rgba(0,0,0,0.20)"}`,
                          display: "flex", alignItems: "center", justifyContent: "center",
                          transition: "all 0.15s", boxSizing: "border-box" as const,
                        }}>
                          {q2 === opt.val && <span style={{ width: 8, height: 8, borderRadius: "50%", background: GREEN }} />}
                        </span>
                  ...
                  <div style={{
                    ...
                    <label htmlFor="q2b" style={{ fontSize: 13, fontWeight: 600, color: GREEN, display: "block", marginBottom: 8 }}>
                      What specifically?
                    </label>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
                    {(["Yes", "No"] as const).map((v) => (
                      <button key={v} type="button" onClick={() => setQ4(v.toLowerCase())} style={ynBtnStyle(q4 === v.toLowerCase())}>
                        {v}
                      </button>
                    ))}
                  </div>
