      ...
      <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
        <div style={{ flexShrink: 0 }}>
          <PieChart width={150} height={150} style={{ overflow: "visible" }}>
            <Pie data={slices} dataKey="total_appearances" cx={70} cy={70} innerRadius={38} outerRadius={65} paddingAngle={slices.length > 1 ? 2 : 0} labelLine={false} label={(props) => <PieSliceLabel {...props} />}>
              {slices.map(r => <Cell key={r.brand} fill={colorMap[r.brand]} />)}
            </Pie>
            {/* Tooltip omitted for single-brand slices — Recharts renders a stuck tooltip artefact on 100% donuts */}
            {slices.length > 1 && (
    ...
    <div className="max-w-7xl mx-auto px-6 pb-0" style={{ display: "flex", flexDirection: "column", gap: 20, paddingBottom: 80 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                          {rows.map(r => {
                            ...
                            const consensus = (r.runs_agreeing != null && r.runs_total != null && r.runs_total > 0)
                              ? `${r.runs_agreeing} of ${r.runs_total} model runs confirmed this.`
                              : null;
                            return (
                              <div key={r.brand_name}>
                                ...
                                <div style={{ paddingLeft: 178, marginTop: 5 }}>
                                  {ev && <p style={{ fontSize: 15, color: "#000", lineHeight: 1.6, margin: 0 }}>{ev}{consensus ? ` ${consensus}` : ""}</p>}
                                </div>
                              </div>
                            );
                          })}
      ...
      <div style={{ background: "#fff", borderRadius: 10, boxShadow: "0 2px 8px rgba(0,0,0,0.07), 0 1px 2px rgba(0,0,0,0.04)", overflow: "hidden" }}>
        <div style={{ padding: "16px 24px", borderBottom: "1px solid rgba(0,0,0,0.07)" }}>
          ...
          <p style={{ fontSize: 15, color: "#888", margin: "5px 0 0" }}>Sourced from <a href="https://parallel.ai" target="_blank" rel="noopener noreferrer" style={{ color: "#888", textDecoration: "underline" }}>parallel.ai</a></p>
        </div>
        <div style={{ padding: "20px 24px" }}>
