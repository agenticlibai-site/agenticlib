...
const radioOptStyle = (selected: boolean): React.CSSProperties => ({
  ...
  gap: 10,
  padding: "10px 12px",
  border: `1.5px solid ${selected ? ORANGE : "rgba(0,0,0,0.12)"}`,
  borderRadius: 8,
  cursor: "pointer",
  background: selected ? "rgba(234,88,12,0.06)" : "transparent",
  transition: "border-color 0.15s, background 0.15s",
});

// ── Component ─────────────────────────────────────────────────────────────────
export default function DexifyFeedbackPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/dexify-feedback", {
        method: "POST",
      ...
      <div style={{ width: "100%", maxWidth: 580 }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 14, color: "#555", margin: 0, lineHeight: 1.55 }}>
            Three questions. Takes about 2 minutes. Answers go directly to the team.
          </p>
        </div>
          ...
          >
            ...
            >
              <svg width="26" height="26" viewBox="0 0 26 26" fill="none">
                <path d="M6 13l5 5 9-10" stroke={ORANGE} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
            <h2 style={{ fontSize: 22, fontWeight: 800, color: "#000", letterSpacing: "-0.02em", marginBottom: 10 }}>
              Thank you, {company || "there"}
            </h2>
            ...
            >
              ...
              <div style={qStyle}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <label htmlFor="company" style={{ ...labelStyle, fontSize: 13, fontWeight: 600, color: ORANGE, letterSpacing: "0.04em", textTransform: "uppercase" as const }}>
                    Company
                  </label>
                  <input
                    id="company"
              ...
              <div style={qStyle}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <textarea id="q1" value={q1} onChange={(e) => setQ1(e.target.value)}
                    placeholder="Anything — a score, a finding, a pattern..." rows={3} style={taStyle} />
                </div>
              </div>
                    ...
                    {[
                      ...
                      <label key={opt.val} style={radioOptStyle(q2 === opt.val)}>
                        <span style={{
                          ...
                          border: `2px solid ${q2 === opt.val ? ORANGE : "rgba(0,0,0,0.20)"}`,
                          display: "flex", alignItems: "center", justifyContent: "center",
                          transition: "all 0.15s", boxSizing: "border-box" as const,
                        }}>
                          {q2 === opt.val && <span style={{ width: 8, height: 8, borderRadius: "50%", background: ORANGE }} />}
                        </span>
                        <input type="radio" name="q2" value={opt.val} checked={q2 === opt.val}
                          onChange={() => setQ2(opt.val)} style={{ display: "none" }} />
                        <span style={{ fontSize: 14, color: "#111", lineHeight: 1.4 }}>{opt.text}</span>
                      </label>
                    ))}
                  </div>
                  <div style={{
                    marginTop: showFollowup ? 14 : 0, maxHeight: showFollowup ? 180 : 0,
                    opacity: showFollowup ? 1 : 0, overflow: "hidden",
                    transition: "max-height 0.25s ease, opacity 0.2s ease, margin-top 0.2s ease",
                  }}>
                    <label htmlFor="q2b" style={{ fontSize: 13, fontWeight: 600, color: ORANGE, display: "block", marginBottom: 8 }}>
                      What specifically?
                    </label>
                    <textarea id="q2b" value={q2b} onChange={(e) => setQ2b(e.target.value)}
                      placeholder="What are you planning to do?" rows={2} style={{ ...taStyle, marginTop: 0 }} />
                  </div>
                </div>
              </div>
              ...
              {/* Submit */}
              <div style={{
                padding: "20px 32px 28px", borderTop: "1px solid rgba(0,0,0,0.07)",
                ...
                <button type="submit" disabled={submitting} style={{
                  ...
                  boxShadow: `0 2px 0 ${ORANGE_DARK}`,
                  opacity: submitting ? 0.65 : 1, fontFamily: "inherit",
