export const metadata = {
  title: "Sales Intelligence – AgenticLib",
  description: "Track your sales AI agent's features, visibility, and sentiment against competitors.",
};

export default function SalesVisibilityPage() {
  return (
    <main className="min-h-screen">

      <div className="max-w-4xl mx-auto px-8 py-20 text-center">
        <div
          className="inline-block text-xs font-semibold px-3 py-1 rounded-full mb-6"
          style={{ background: "rgba(37,99,235,0.10)", color: "#2563EB" }}
        >
          AI Agent Builder · Sales
        </div>
        <h1 className="text-4xl md:text-5xl font-bold mb-6" style={{ color: "#000000", letterSpacing: "-0.02em", lineHeight: 1.1 }}>
          Comparison Intelligence for Sales AI Agents
        </h1>
        <p className="text-lg max-w-xl mx-auto" style={{ color: "#000000", lineHeight: 1.6 }}>
          Track your sales AI agent&apos;s features, visibility, and sentiment against competitors.
        </p>
        <p className="mt-16 text-sm" style={{ color: "#000000" }}>Coming soon</p>
      </div>
    </main>
  );
}
