...
const EVIDENCE_OVERRIDE: Record<string, Record<string, string>> = {
  ...
  cost_free_trial: {
    "Reply.io": "Reply.io offers a 14-day free trial with no credit card required. The trial provides access to core platform features including a B2B database, multichannel sequences (email, LinkedIn, SMS, calls), reports, analytics, and API access with integrations like Salesforce and HubSpot. Users can start the trial directly at reply.io without requiring a sales call or demo, and pricing stays transparent beyond the trial too. Three published tiers: $49/user (Starter, email-only), $89/user (Professional, adds LinkedIn, calling, SMS), $139/user (Ultimate, full AI SDR). Note that advertised base prices exclude add-ons — most teams running multichannel outreach land at $150–187/user once LinkedIn automation ($69) and cloud calling ($29) are added. That published, self-serve pricing is still worth copying regardless of trial length; it lets a buyer size up real cost before ever talking to sales.",
