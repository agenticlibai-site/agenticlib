...
const BRAND_FEATURE_DESCRIPTIONS: Record<string, Record<string, string>> = {
  recording_no_install: {
    "Descript":     "Descript records screen and webcam via its desktop app, generating an automatic transcript the moment recording stops. The session lands directly in the project timeline — no import step — ready for text-based editing.",
    "HeyGen":       "HeyGen offers in-browser screen capture with a webcam overlay and no extension required. Recordings import automatically into the HeyGen editor for further production.",
    "Opus Clip":    "Opus Clip does not capture screen sessions natively — users record externally and upload. Its value is in post-processing and clip generation, not capture itself.",
    "Synthesia":    "Synthesia does not offer screen recording. It generates avatar presenter videos from written scripts rather than capturing real-world footage.",
    "D-ID":         "D-ID does not offer screen recording. The platform generates talking-head avatar videos from photographs and scripts.",
    "DeepBrain":    "DeepBrain is avatar-centric and does not include a native screen recording tool. Videos are produced from text scripts, not live capture sessions.",
    "Renderforest": "Renderforest does not include screen recording capability. Videos are assembled from templates and uploaded media assets.",
  },
  recording_upload_support: {
    "Descript":     "Descript accepts all major video and audio formats on upload, generates a full transcript, and places each file on a multi-track timeline with separate audio, video, and screen layers.",
    "Opus Clip":    "Opus Clip is built around imported long-form video. It accepts YouTube links, direct uploads, and recording imports as the primary input for its clip-generation workflow.",
    "HeyGen":       "HeyGen accepts uploaded video for its Video Translate and Video Avatar features. Importing existing footage is central to HeyGen's repurposing and dubbing workflows.",
    "D-ID":         "D-ID accepts uploaded images and short video clips for face-animation and avatar-overlay workflows. Video import supports re-animation rather than full-length editing.",
    "Synthesia":    "Synthesia's primary input is a written script, not uploaded video. Some media upload is supported for scene backgrounds and assets, but footage editing is not the core workflow.",
    "DeepBrain":    "DeepBrain generates videos from scripts and avatar selections rather than processing uploaded footage. Media imports are limited to background images and assets.",
    "Renderforest": "Renderforest supports media uploads for use within template slots, but is not designed for importing raw footage for editorial-style editing.",
  },
  production_auto_zoom: {
    "Opus Clip":    "Auto-zoom is Opus Clip's signature capability — the platform dynamically centres on the speaker and zooms to emphasise energy and speech intensity during clip generation. No manual keyframing is needed.",
    "Descript":     "Underlord can apply zoom effects timed to transcript events such as topic shifts or high-energy phrases. Zoom is derived from the text rather than manual keyframe placement.",
    "HeyGen":       "HeyGen applies camera movement and zoom presets to avatar presenter videos to simulate a more dynamic recording. It does not apply speaker-tracking zoom to imported real-world footage.",
    "Synthesia":    "Synthesia's avatar scenes use fixed or pre-set camera angles. Zoom-style effects are applied at the scene level through template configuration rather than automatic content analysis.",
    "D-ID":         "D-ID applies controlled camera movements to avatar-generated content. Automatic speaker-tracking zoom on imported footage is not a supported feature.",
    "DeepBrain":    "DeepBrain supports camera movement presets within avatar scene templates. Dynamic zoom based on content analysis is not a core feature.",
    "Renderforest": "Camera movement and zoom effects are available within Renderforest templates, but they are fixed to the template design rather than applied automatically based on video content.",
  },
  production_transitions: {
    "Descript":     "Underlord analyses pacing and filler words to suggest cut points rather than generating stylistic transitions. Standard transition effects are available in the timeline editor for manual application.",
    "Opus Clip":    "Clip boundaries and transition styles are selected automatically during the repurposing workflow, chosen to maintain energy and flow between successive cuts.",
    "HeyGen":       "Scene transitions between avatar segments are available within HeyGen's video editor. Templates include pre-set transition styles for professional presentations.",
    "Synthesia":    "Synthesia's multi-scene format includes defined entry and exit points per scene, with transition style options configurable within the script and scene editor.",
    "D-ID":         "Transitions between avatar scenes can be configured within the Creative Reality Studio interface. Options are more limited than full-featured video editors.",
    "DeepBrain":    "Pre-defined transition styles are available between scenes in DeepBrain's multi-scene avatar video editor. Template sets include matching transition themes.",
    "Renderforest": "A library of transition styles is available within the Renderforest template editor, applied between video segments and motion graphics sequences.",
  },
  voice_cloning: {
    "Descript":     "Descript's Overdub creates a text-to-speech clone of a speaker's voice from a recorded sample. Any future script edit can be re-narrated in the cloned voice without a new recording session.",
    "HeyGen":       "HeyGen Voice Clone generates a branded AI voice from a short audio sample. The cloned voice narrates any future script consistently, eliminating repeated recording for video variants.",
    "Synthesia":    "Synthesia supports custom AI voice creation and voice cloning through its enterprise tier. Cloned voices are deployed consistently across an organisation's avatar video library.",
    "D-ID":         "D-ID supports voice synthesis alongside its avatar generation, allowing a cloned or custom voice to drive a photorealistic talking-head in sync. Voice and visual are rendered together.",
    "DeepBrain":    "DeepBrain offers custom AI voice creation as part of its avatar production workflow. Each voice clone is associated with a specific avatar persona for consistent output.",
    "Opus Clip":    "Opus Clip does not natively support voice cloning. The platform uses existing audio from uploaded recordings rather than generating new synthesised speech.",
    "Renderforest": "Renderforest does not offer voice cloning. Text-to-speech functionality uses pre-built voice library options rather than custom clones.",
  },
  voice_talking_head: {
    "Synthesia":    "Synthesia's core product is a library of 240+ AI presenter avatars that lip-sync to any script with realistic expressions and body language. No camera or human presenter is required.",
    "HeyGen":       "HeyGen generates photorealistic avatar presenters from a short video sample or from its pre-built library, with accurate lip sync across 175+ languages and matching facial expressions.",
    "D-ID":         "D-ID animates a single still photograph into a realistic talking-head video. This enables avatar creation from one photo without any video recording.",
    "DeepBrain":    "DeepBrain's core product is enterprise-grade photorealistic AI avatars. Avatars present any script at broadcast-quality resolution with natural lighting and expression.",
    "Descript":     "Descript includes an AI avatar and Green Screen feature, but talking-head avatar generation is a secondary capability compared to its primary text-based editing workflow.",
    "Opus Clip":    "Opus Clip repurposes existing video recordings rather than synthesising new presenter content. Talking-head avatar generation is not a supported feature.",
    "Renderforest": "Renderforest provides animated characters within certain templates but does not support photorealistic talking-head avatar generation from scripts.",
  },
  captions_auto: {
    "Descript":     "Captions are generated automatically from the project transcript on every recording. They are fully editable in the transcript view before export, and styled to match brand guidelines.",
    "Opus Clip":    "Auto-captions are generated on every clip with animated word-by-word highlighting designed for social viewing. Caption accuracy is high because Opus Clip optimises this as a core output.",
    "HeyGen":       "Auto-captions are generated for both avatar videos and imported footage. HeyGen offers caption styling and animation options for social-ready output.",
    "Synthesia":    "Captions are generated from the known script and can be toggled on or off per video. Multi-language caption display is aligned with the dubbed audio track.",
    "DeepBrain":    "Auto-captions are derived from the script used to produce the avatar video, so accuracy is high — the text source is known rather than inferred from audio.",
    "Renderforest": "Captions can be added as text overlays within templates. Automatic speech-to-text captioning is limited compared to tools built around transcription.",
    "D-ID":         "Caption support is available but secondary to D-ID's avatar video generation workflow. Captions are added as post-processing text overlays.",
  },
  captions_styling: {
    "Opus Clip":    "Caption styling is one of Opus Clip's strongest features — font, colour, size, animation, and word-highlight style are all customisable. Branded presets can be saved and reused across every clip.",
    "Descript":     "Caption styling supports font and colour customisation within the layout editor. Options are sufficient for branded content but less extensive than dedicated social tools.",
    "HeyGen":       "Caption style options within HeyGen's editor include font, colour, and position, with social-optimised presets for common platform formats.",
    "Synthesia":    "Captions support language-specific font rendering and can be styled within the scene template. Enterprise templates enforce consistent caption branding across the team workspace.",
    "Renderforest": "Text style options are available through the template editor. Caption design flexibility is tied to the chosen template rather than fully freeform.",
    "DeepBrain":    "Basic font and colour options for captions are supported within DeepBrain's video editor. Styling options are present but limited.",
    "D-ID":         "Caption styling in D-ID is limited. The primary output is the avatar video; caption presentation is a secondary and less developed feature.",
  },
  translation_languages: {
    "Synthesia":    "Synthesia generates avatar videos natively in 130+ languages with localised voiceovers and lip-synced avatars. No source video re-recording is required — the same script renders in any language from a single production pass.",
    "HeyGen":       "HeyGen Video Translate converts an existing video into 175+ languages, preserving the original speaker's voice, lip movements, and expressions with accurate lip-syncing to the new audio.",
    "Opus Clip":    "Opus Clip translates video captions into 30+ languages. Translation is subtitle-only — the audio track is not dubbed, so the original speech remains unchanged.",
    "Descript":     "Descript transcribes in multiple languages but full AI dubbing into other languages is limited. Translation is primarily a transcript-level feature rather than audio or lip-sync replacement.",
    "D-ID":         "D-ID supports video translation and dubbing with avatar lip-movements re-generated in the target language. The avatar speaks the translated audio natively rather than post-dubbing.",
    "DeepBrain":    "DeepBrain generates multilingual avatar videos by narrating the script in the chosen language. It regenerates the avatar's speech rather than dubbing existing footage.",
    "Renderforest": "Renderforest does not offer automated video translation or audio dubbing. Language variants require manual re-creation within the template editor.",
  },
  translation_narration_regen: {
    "Synthesia":    "Narration is regenerated natively for each language — the avatar speaks the target language from the script rather than post-dubbing. This produces authentic lip sync and natural intonation without a second recording.",
    "HeyGen":       "HeyGen regenerates narration in the target language using the original speaker's cloned voice, maintaining consistent voice identity and natural expression across all language versions.",
    "D-ID":         "D-ID re-generates both the avatar's speech and lip movements in the target language, producing a native-sounding translated version from the same avatar identity and source script.",
    "DeepBrain":    "DeepBrain generates new narration in the target language from the same avatar, with both voice and lip animation updated to match the translated script.",
    "Descript":     "Overdub can re-narrate script changes in a cloned voice but cross-language narration regeneration is limited. Full multilingual dubbing with lip sync is not a current core capability.",
    "Opus Clip":    "Opus Clip does not regenerate narration for translation. The source audio track is not replaced — only subtitles are translated.",
    "Renderforest": "Renderforest does not support narration regeneration for translation. Audio tracks must be replaced manually for language variants.",
  },
  branding_brand_kit: {
    "Descript":     "Descript's Brand Studio stores colours, fonts, and logo assets for automatic application across all video projects. Templates created within a workspace are stamped with brand identity from the start.",
    "Synthesia":    "Enterprise accounts apply a brand kit — logo, fonts, and colour palette — to all avatar video templates. Brand consistency is enforced at the workspace level across every team member.",
    "HeyGen":       "HeyGen's Brand Kit stores logo, colours, and fonts for automatic application across templates, available on Business and Enterprise plans.",
    "Renderforest": "Brand kits are one of Renderforest's strongest capabilities — logos, colours, and fonts are stored and applied automatically to template-based designs across the full template library.",
    "Opus Clip":    "Branding elements including logo, colours, and intro/outro sequences are saved and applied automatically to every generated clip, ensuring consistency across all social content.",
    "DeepBrain":    "Brand kits covering background colours, logo placement, and font selection for titles and lower-thirds can be applied to avatar video templates across the workspace.",
    "D-ID":         "Background and overlay customisation is available, but D-ID's brand kit feature is less comprehensive than dedicated video production platforms. Custom branding requires more manual configuration.",
  },
  branding_templates: {
    "Renderforest": "Renderforest's core product is a library of thousands of animated video templates covering explainer videos, social content, presentations, and logo animations. Template variety is the platform's primary strength.",
    "Synthesia":    "Synthesia provides a library of professionally designed templates with scene layouts, avatar positioning, and background options. Enterprise teams can create and lock custom templates for their brand.",
    "HeyGen":       "HeyGen's template library covers avatar presenter layouts for training, marketing, and social content. Templates are fully customisable within the editor.",
    "Descript":     "Descript includes layout templates and intro/outro presets for framing recordings. Template options are simpler than dedicated template platforms but sufficient for branded content workflows.",
    "Opus Clip":    "Social content templates for formatted clips — including animated captions, hook text, and CTA overlays — are available, optimised for vertical video and platform-specific formats.",
    "DeepBrain":    "DeepBrain includes template sets for training and corporate communication scenarios, covering stage design, background, and avatar layout for common use cases.",
    "D-ID":         "Scene and background templates are available for avatar video production, but template variety is more limited than dedicated template-first platforms.",
  },
  agents_autonomous_record: {
    "Descript":     "Underlord performs editing actions autonomously — trimming filler words, shortening gaps, and applying transcript corrections — but does not autonomously initiate or conduct a recording session without a human present.",
    "Synthesia":    "Synthesia does not offer an autonomous recording agent. Video creation starts from a human-authored script that the platform renders with an AI avatar.",
    "HeyGen":       "HeyGen's Video Agent API enables programmatic video generation from structured inputs, but a human must define the content and trigger the request. Autonomous session initiation is not supported.",
    "Opus Clip":    "Opus Clip does not autonomously record. Agent functionality focuses on repurposing footage that has already been recorded by a human rather than initiating sessions independently.",
    "D-ID":         "D-ID does not offer an autonomous recording agent. Videos are generated on demand from provided scripts and images rather than through an agent that initiates sessions independently.",
    "DeepBrain":    "DeepBrain does not offer autonomous agent-driven recording. A human must provide the script before video generation begins.",
    "Renderforest": "Renderforest does not offer autonomous recording or agent-driven production capability of any kind.",
  },
  agents_safety: {
    "Synthesia":    "Synthesia publishes an AI ethics policy and restricts avatar creation to consented individuals. It screens content for misuse and maintains a Trust & Safety programme covering deepfake prevention and responsible use standards.",
    "HeyGen":       "HeyGen requires explicit creator consent for voice cloning and avatar generation. It operates an abuse prevention programme and complies with EU AI Act disclosure requirements for AI-generated content.",
    "Descript":     "Descript requires a spoken consent statement to create an Overdub voice clone. Usage policies restrict the cloned voice to the consenting person's own content, with enforcement built into the creation flow.",
    "D-ID":         "D-ID has published ethical guidelines for its avatar and face-animation technology, prohibiting generation of content impersonating real individuals without their consent.",
    "Opus Clip":    "Opus Clip's safety controls focus on general content moderation rather than AI-generation governance. No specific voice-clone or avatar consent framework is publicly documented.",
    "DeepBrain":    "DeepBrain publishes guidelines around consent for custom avatar creation but does not have a publicly documented safety programme equivalent in depth to Synthesia's or HeyGen's.",
    "Renderforest": "Renderforest does not produce AI-generated avatars or voice clones, so avatar consent and AI safety governance frameworks are not applicable. Standard content policy applies.",
  },
  distribution_embed: {
    "Descript":     "Published Descript videos are accessible via a hosted link or embeddable iframe. The hosted player includes transcript display, chapter navigation, and clip-level sharing.",
    "Synthesia":    "Videos are hosted on Synthesia's platform and shareable via a branded player link. Embed codes are available for inserting videos into web pages, LMS platforms, and internal portals.",
    "HeyGen":       "Generated videos can be downloaded or shared via a hosted link. Embed codes are available for inserting content into websites and external platforms.",
    "Renderforest": "Renderforest videos can be shared via a hosted link or downloaded for distribution. Embed codes are supported for website integration.",
    "Opus Clip":    "Clips can be published directly to connected social platforms or downloaded. A hosted link is available but the workflow is primarily direct-to-social rather than embed-first.",
    "D-ID":         "Generated videos are downloadable and shareable via link. Studio videos can be embedded, though D-ID is more commonly used for API-driven delivery rather than hosted player scenarios.",
    "DeepBrain":    "Completed videos are downloadable and shareable via a hosted link. A player embed option is available for sharing avatar videos on external pages.",
  },
  distribution_analytics: {
    "Synthesia":    "Synthesia tracks view counts, watch time, and completion rates per video. Enterprise accounts get team-level analytics across the full video library, enabling L&D teams to measure training engagement.",
    "Descript":     "Basic view analytics are available for published Descript videos. The analytics surface covers views and watch time but is simpler than dedicated video hosting platforms.",
    "HeyGen":       "View counts and basic engagement metrics are available for shared HeyGen videos. More detailed analytics are unlocked on higher-tier plans.",
    "Renderforest": "Basic download and view metrics are surfaced within the Renderforest dashboard. Analytics depth is limited compared to dedicated video hosting or analytics tools.",
    "Opus Clip":    "Opus Clip tracks social performance metrics — views, likes, shares — on clips published to connected social accounts. Analytics reflect social platform data rather than hosted player metrics.",
    "D-ID":         "Basic view metrics are available for videos shared via the D-ID platform. Analytics are not a primary feature of the product.",
    "DeepBrain":    "View metrics are available for videos hosted on the DeepBrain platform. Analytics depth is limited for a video production tool and does not include engagement-level data.",
  },
  collab_team_workspace: {
    "Descript":     "Team plans include a shared workspace where members collaborate on projects with role-based access. Creator and Editor roles determine who can publish and who can edit.",
    "Synthesia":    "Enterprise accounts provide a shared workspace with team management, role-based access, and the ability to assign videos and custom templates across the team for consistent output.",
    "HeyGen":       "Team and Enterprise plans offer a shared video library, role assignments, and brand kit access across all team members. Workspace management supports multi-department rollout.",
    "Renderforest": "Team collaboration is supported through shared brand kits and project folders with role-based permissions on team plans.",
    "Opus Clip":    "Team workspace features enable shared clip libraries and consistent brand settings across members. Collaboration is centred on output consistency rather than real-time co-editing.",
    "D-ID":         "Organisational accounts allow shared access to generated content and API credentials. RBAC is available for enterprise customers managing multiple users.",
    "DeepBrain":    "Team accounts provide shared access to avatar libraries, template sets, and completed videos. Role-based workspace management is available for enterprise deployments.",
  },
  collab_review: {
    "Descript":     "Collaborators with view or edit access can leave timestamped comments directly on the video timeline. Comments are marked resolved when the edit is applied, creating a clear feedback loop.",
    "HeyGen":       "Review links allow external stakeholders to view and comment on a video before it is finalised, without requiring a HeyGen account. Feedback is collected in a single accessible view.",
    "Synthesia":    "Sharing links support stakeholder review. Timestamped commenting within the Synthesia platform is available for workspace collaborators on shared video projects.",
    "Renderforest": "Projects can be shared for feedback, but in-platform timestamped review is limited. Most review workflows occur via external communication tools.",
    "Opus Clip":    "Clips can be shared for review before publishing to social platforms. Direct timestamped commenting within the Opus Clip platform is not a primary feature.",
    "D-ID":         "Review links allow stakeholder sign-off before finalising a video. In-platform collaborative commenting is limited; review primarily occurs via external tools.",
    "DeepBrain":    "Review links allow sharing of completed videos for stakeholder approval. In-platform commenting is limited and review workflows typically happen outside the platform.",
  },
  editor_timeline: {
    "Descript":     "Descript's timeline supports multi-track audio and video with text-based editing as the primary interface. Edits in the transcript propagate to the timeline automatically, and direct timeline manipulation is also available for fine-grained control.",
    "HeyGen":       "HeyGen includes a scene-based video editor for arranging avatar segments, overlays, and imported media. The timeline is suited to presentation-style videos rather than frame-level editorial work.",
    "Opus Clip":    "Opus Clip has a lightweight clip editor for trimming and adjusting generated clips. It is optimised for quick social edits rather than complex multi-track production.",
    "Synthesia":    "Synthesia's editor is scene-based and used for arranging scripted avatar presentations. It is not designed for traditional multi-track timeline editing of recorded footage.",
    "D-ID":         "D-ID's editor focuses on avatar configuration rather than timeline-based video editing. Multi-track production is not a core capability of the platform.",
    "DeepBrain":    "DeepBrain includes a scene-level editor for arranging avatar segments. Full multi-track timeline editing of recorded footage is not a primary feature.",
    "Renderforest": "Renderforest's template-based editor allows segment reordering and media replacement within template slots. Traditional frame-level multi-track timeline editing is not supported.",
  },
  editor_text_effects: {
    "Descript":     "Text overlays, titles, and callout cards are available within Descript's layout editor. Animation options for lower-thirds and title cards are included, covering common branded content needs though fewer options than dedicated motion graphics tools.",
    "Opus Clip":    "Animated captions and text hooks are a core Opus Clip feature designed for social video. Word-by-word caption animation, hook text overlays, and CTA callouts are all available and customisable.",
    "Renderforest": "Animated text effects are a primary strength of Renderforest's template library. Many templates feature kinetic typography, logo reveals, and text animation sequences.",
    "HeyGen":       "HeyGen includes text overlay and animated title options within its video editor, designed for avatar presentation videos rather than editorial-style motion graphics.",
    "Synthesia":    "Text overlays, lower-thirds, and title cards are available within Synthesia's scene templates. Animation options are template-defined and consistent across the workspace rather than fully freeform.",
    "D-ID":         "Basic text overlay capability is available in D-ID. Animated text effects are limited within the primary avatar video workflow.",
    "DeepBrain":    "Text overlays and lower-thirds are supported within DeepBrain's video editor. Animation options for text are present but more limited than platforms specialising in editorial video production.",
  },
};
        ...
        <div style={{ background: "#fff", borderRadius: 10, boxShadow: "0 2px 8px rgba(0,0,0,0.07), 0 1px 2px rgba(0,0,0,0.04)", overflow: "hidden" }}>
          ...
          <div style={{ padding: "20px 24px" }}>
            ...
            <div style={{ borderTop: "1px solid rgba(0,0,0,0.08)", margin: "8px 0 24px", paddingTop: 24 }}>
              <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.09em", textTransform: "uppercase" as const, color: "rgba(0,0,0,0.35)", marginBottom: 20 }}>
                Research-sourced · based on public documentation at time of research
              </p>
              {[
                {
                  id: "pricing",
