...
export async function POST(req: Request) {
  try {
    ...
    console.log("[RALFI-FEEDBACK]", JSON.stringify({ ...data, _logged: new Date().toISOString() }, null, 2));
    ...
    await transporter.sendMail({
      ...
      subject: `Ralfi Report Feedback — ${company}`,
      html: `
        <h2 style="font-family:sans-serif;color:#059669;margin:0 0 4px;">Insurance Broker Report Feedback</h2>
        ...
        <table style="font-family:sans-serif;border-collapse:collapse;width:100%;max-width:560px;">
          ...
          <tr>
            <td style="padding:8px 12px;font-weight:600;color:#3f3f46;background:#f4f4f5;">Want report monthly?</td>
            <td style="padding:8px 12px;color:#18181b;">${data.q4_want_monthly || "—"}</td>
          </tr>
        </table>
      `,
    });
    ...
    console.error("[RALFI-FEEDBACK] email error:", err);
