...
const cache = new Map<string, { data: BrandIntelligenceData; timestamp: number }>();
...
// Normalise aliases to canonical display name
...
function parseResponse(text: string, useCase: string, promptText: string, llm: LLMKey): MentionRecord[] {
  ...
  for (const agent of TRACKED_AGENTS) {
    ...
    // 500-character window centred on the mention
...
function aggregate(allMentions: MentionRecord[], totalPrompts: number): AgentResult[] {
  ...
  const totalCombinations = totalPrompts * 2; // Claude + GPT-4o
  ...
  for (const [name, mentions] of byAgent) {
    ...
    // Real indicator words extracted from LLM responses — no fallback
    const descriptors = [...new Set(mentions.flatMap(m => m.foundWords))].slice(0, 4);

    // Use cases where this agent was mentioned first (rank=1), deduped by useCase label
    ...
    const topUseCases = [...topUseCasesMap.values()];
...
export async function GET() {
  ...
  const agents = aggregate(mentionsByPrompt.flat(), USE_CASE_PROMPTS.length);

  const data: BrandIntelligenceData = {
    domain: "marketing",
