...
import {
  HSAI_SENTIMENT_CLUSTERS, HSAI_SENTIMENT_SYSTEM_PROMPT, buildHsaiSentimentPrompt,
} from "@/lib/brand-visibility/hsai-sentiment";
import { initHsaiDB, getHsaiBrandsForFeatureScoring, insertHsaiSentimentResponse } from "@/lib/brand-visibility/db";
import { sendEmail } from "@/lib/email";

export const dynamic     = "force-dynamic";
export const maxDuration = 300;

// ?model=claude-haiku-4-5  →  10:10 UTC daily
// ?model=gpt-4o-mini       →  10:40 UTC daily
// Weekly aggregation: hsai-sentiment-aggregate, Sunday 04:30 UTC.
...
async function callClaude(promptText: string): Promise<string> {
  const res = await anthropic.messages.create({
    ...
    system: HSAI_SENTIMENT_SYSTEM_PROMPT,
...
async function callGPT(promptText: string): Promise<string> {
  const res = await openai.chat.completions.create({
    ...
    messages: [
      { role: "system", content: HSAI_SENTIMENT_SYSTEM_PROMPT },
...
function parseSentimentResponse(raw: string): {
  ...
  function extract(obj: Record<string, unknown>) {
    // limitations is TEXT in HSAI (not TEXT[]), so coerce to string
    const lim = obj.limitations;
    const limitationsStr = Array.isArray(lim) ? (lim as string[]).join("; ")
      : typeof lim === "string" ? lim : null;
    return {
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (brands.length === 0) {
      return Response.json({ mode: "hsai_sentiment_collection", date: today, note: "no brands in LOCKED_HSAI_BRANDS yet" });
    }

    const tasks: (() => Promise<{ success: boolean }>)[] = [];
    for (const brandName of brands) {
      for (const cluster of HSAI_SENTIMENT_CLUSTERS) {
        const b = brandName, c = cluster;
        tasks.push(async () => {
          try {
            const promptText = buildHsaiSentimentPrompt(b, c);
            ...
            await insertHsaiSentimentResponse({
              brand_name: b, prompt_id: c.id, bucket_tag: c.bucket_tag,
            ...
            console.error(`[hsai-sentiment-collection] error: ${brandName}/${cluster.bucket_tag}:`, err);
    ...
    console.log(`[hsai-sentiment-collection] start — model=${model}, date=${today}, brands=${brands.length}, tasks=${expected}`);
    const results   = await runWithConcurrency(tasks, BATCH_CONCURRENCY, BATCH_DELAY_MS);
    const succeeded = results.filter((r) => r.success).length;
    const failed    = expected - succeeded;
    console.log(`[hsai-sentiment-collection] done — succeeded=${succeeded}/${expected}, failed=${failed}`);
    ...
    return Response.json({ mode: "hsai_sentiment_collection", model, date: today, brands: brands.length, expected, succeeded, failed });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`[cron] hsai-sentiment-collection crashed (${model}):`, message);
