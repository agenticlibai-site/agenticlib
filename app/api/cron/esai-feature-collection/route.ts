...
import {
  ESAI_FEATURES, ESAI_FEATURE_SYSTEM_PROMPT, buildEsaiFeaturePrompt, type EsaiFeature,
} from "@/lib/brand-visibility/esai-features";
import { initEsaiDB, getEsaiTopBrandsForFeatureScoring, insertEsaiFeatureResponse } from "@/lib/brand-visibility/db";
import { sendEmail } from "@/lib/email";

export const dynamic     = "force-dynamic";
export const maxDuration = 300;

// 20 features × brands × 3 runs = calls per model.
// ?model=claude-haiku-4-5  →  11:05 UTC daily
// ?model=gpt-4o-mini       →  11:20 UTC daily
// esai-feature-aggregate runs at 11:55 UTC.
...
async function withRetry<T>(fn: () => Promise<T>, label: string): Promise<T> {
  ...
  for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt++) {
    try { return await fn(); } catch (err) {
      ...
      console.error(`[esai-feature-collection] non-retriable error on ${label}:`, err);
      throw err;
    }
  }
  throw lastErr;
...
async function callClaude(promptText: string): Promise<string> {
  const res = await anthropic.messages.create({
    ...
    system: ESAI_FEATURE_SYSTEM_PROMPT,
...
async function callGPT(promptText: string): Promise<string> {
  const res = await openai.chat.completions.create({
    ...
    messages: [
      { role: "system", content: ESAI_FEATURE_SYSTEM_PROMPT },
...
async function callClaudeGrounded(brandName: string, feature: EsaiFeature): Promise<string> {
  const featurePrompt = buildEsaiFeaturePrompt(feature, brandName);
  const res = await anthropic.messages.create({
    model: "claude-haiku-4-5-20251001", max_tokens: 1024,
    tools: [{ type: "web_search_20250305" as const, name: "web_search" as const }],
    messages: [{
      role: "user",
      content: `Search for information about ${brandName}'s product features for Australian construction estimating, specifically: ${feature.feature_name}. Then answer this question about ${brandName} only:\n\n${featurePrompt}`,
    }],
  });
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (brands.length === 0) {
      return Response.json({ mode: "esai_feature_collection", date: today, note: "no brands in LOCKED_ESAI_BRANDS yet" });
    }

    const pairResults = new Map<string, RunSummary[]>();
    const pairMeta    = new Map<string, { brandName: string; feature: EsaiFeature }>();
    const tasks: (() => Promise<{ success: boolean }>)[] = [];

    for (const brandName of brands) {
      for (const feature of ESAI_FEATURES) {
        ...
        for (let run = 1; run <= RUNS_PER_FEATURE; run++) {
          ...
          tasks.push(async () => {
            try {
              const promptText = buildEsaiFeaturePrompt(f, b);
              ...
              await insertEsaiFeatureResponse({
              ...
              console.error(`[esai-feature-collection] error: ${brandName}/${feature.feature_id}/run${run}:`, err);
              if (!pairResults.has(pairKey)) pairResults.set(pairKey, []);
    ...
    console.log(`[esai-feature-collection] start — model=${model}, date=${today}, brands=${brands.length}, features=${ESAI_FEATURES.length}, tasks=${expected}`);
    ...
    if (model === "claude-haiku-4-5") {
      ...
      for (const [pairKey, runs] of pairResults) {
        ...
        groundingTasks.push(async () => {
          try {
            ...
            console.error(`[esai-feature-collection] grounding error: ${b}/${f.feature_id}:`, err);
            groundingFailed++;
          }
        });
      }
      if (groundingTasks.length > 0) {
        ...
        if (elapsedMs > GROUNDING_DEADLINE_MS) {
          console.warn(`[esai-feature-collection] grounding skipped — ${Math.round(elapsedMs / 1000)}s elapsed (${groundingTasks.length} pairs deferred)`);
        } else {
    ...
    if (failed > 0) {
      await sendEmail({
        subject: `[FeatureStream] ALERT — ESAI Feature Collection failed (${model}, ${today})`,
        html: `<h2>ESAI Feature Pipeline — Failures</h2><p>Model: ${model} | Date: ${today} | Succeeded: ${succeeded}/${expected} | Failed: ${failed}</p>`,
      }).catch(() => {});
    }

    return Response.json({ mode: "esai_feature_collection", model, date: today, brands: brands.length, features: ESAI_FEATURES.length, expected, succeeded, failed, grounding_ran: groundingRan, grounding_failed: groundingFailed });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`[cron] esai-feature-collection crashed (${model}):`, message);
