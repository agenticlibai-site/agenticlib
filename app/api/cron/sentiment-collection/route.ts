...
// ── Split-job design ───────────────────────────────────────────────────────────
// ~49 calls per model per day (22 brands × avg 2.2 cluster prompts).
//
//   ?model=claude-haiku-4-5  →  8:00 AM UTC
//   ?model=gpt-4o-mini       →  2:00 PM UTC  (6h after Claude)
//
// Weekly aggregation runs Sunday 1:00 AM UTC via sentiment-aggregate.
// ──────────────────────────────────────────────────────────────────────────────
...
export async function GET(request: Request) {
  ...
  try {
      ...
      for (const cluster of clusters) {
        ...
        tasks.push(async () => {
          ...
          try {
            ...
            console.error(`[sentiment-collection] error: ${callLabel} (${model}):`, err);
    ...
    if (failed > 0) {
      await sendEmail({
        subject: `[FeatureStream] ALERT — Sentiment Collection failed (${model}, ${today})`,
        html: `
          <h2>Sentiment Pipeline — Collection Failures</h2>
    ...
    console.error(`[cron] sentiment-collection crashed (${model}):`, message);

    await sendEmail({
      subject: `[FeatureStream] CRASH — Sentiment Collection (${model}, ${today})`,
      html: `
