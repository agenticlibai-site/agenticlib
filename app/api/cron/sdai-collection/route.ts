...
import { SDAI_PROMPTS, SDAI_COLLECTION_SYSTEM_PROMPT } from "@/lib/brand-visibility/sdai-prompts";
import { initSdaiDB, insertSdaiRawResponse } from "@/lib/brand-visibility/db";
...
// ── Split-job design ───────────────────────────────────────────────────────────
//   ?model=claude-haiku-4-5  →  06:00 UTC daily
//   ?model=gpt-4o-mini       →  06:10 UTC daily
// sdai-aggregate runs at 06:45 UTC.
...
async function callClaude(promptText: string): Promise<ModelResult> {
  const res = await anthropic.messages.create({
    ...
    system: SDAI_COLLECTION_SYSTEM_PROMPT,
...
async function callGPT(promptText: string): Promise<ModelResult> {
  const res = await openai.chat.completions.create({
    ...
    messages: [
      { role: "system", content: SDAI_COLLECTION_SYSTEM_PROMPT },
...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const prompt of SDAI_PROMPTS) {
    ...
    console.log(`[sdai-collection] start — model=${model}, date=${today}, prompts=${SDAI_PROMPTS.length}, tasks=${expected}`);
    const results   = await runWithConcurrency(tasks, BATCH_CONCURRENCY, BATCH_DELAY_MS);
    const succeeded = results.filter((r) => r.success).length;
    const failed    = expected - succeeded;
    console.log(`[sdai-collection] done — succeeded=${succeeded}/${expected}, failed=${failed}`);

    if (failed > 0) {
      await sendEmail({
        subject: `[FeatureStream] ALERT — SDAI Collection failed (${model}, ${today})`,
        html: `<h2>SDAI Brand Coverage — Collection Failures</h2><p>Model: ${model} | Date: ${today} | Succeeded: ${succeeded}/${expected} | Failed: ${failed}</p>`,
      }).catch(() => {});
    }

    return Response.json({ mode: "sdai_collection", model, date: today, prompts: SDAI_PROMPTS.length, expected, succeeded, failed });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`[cron] sdai-collection crashed (${model}):`, message);
