...
// ── Split-job design ───────────────────────────────────────────────────────────
// 10 features × top-20 brands × 3 runs = 600 API calls per model.
//
//   ?model=claude-haiku-4-5   →  7:00 UTC (all brands, includes grounding pass)
//   ?model=gpt-4o-mini        →  7:15 UTC (all brands)
//
// dexify-aggregate runs at 7:45 UTC (30 min after GPT job).
//
// Optional: ?date=YYYY-MM-DD for historical backfill. ON CONFLICT DO NOTHING is safe.
// ──────────────────────────────────────────────────────────────────────────────
  ...
  for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt++) {
    try {
      ...
      if (status === 429 && attempt < RETRY_DELAYS_MS.length) {
        ...
        console.warn(`[dexify-feature-collection] 429 on ${label} — retry ${attempt + 1}/3 in ${delay}ms`);
...
async function callGPT(promptText: string): Promise<string> {
  const res = await openai.chat.completions.create({
    ...
    messages: [
      { role: "system", content: DEXIFY_FEATURE_SYSTEM_PROMPT },
...
async function callClaudeGrounded(brandName: string, feature: DexifyFeature): Promise<string> {
  const featurePrompt = buildDexifyPrompt(feature, brandName);
  const res = await anthropic.messages.create({
    model:      "claude-haiku-4-5-20251001",
    max_tokens: 1024,
    ...
    messages:   [{
      role:    "user",
      content: `Search for information about ${brandName}'s product features, specifically: ${feature.feature_name}. Then answer this question about ${brandName} only:\n\n${featurePrompt}`,
    }],
  });
  ...
  try {
    ...
    if (brands.length === 0) {
      return Response.json({
        mode:  "dexify_feature_collection",
        date:  today,
    ...
    const pairMeta    = new Map<string, { brandName: string; feature: DexifyFeature }>();
    const tasks: (() => Promise<{ success: boolean }>)[] = [];

    for (const brandName of brands) {
      for (const feature of DEXIFY_FEATURES) {
        ...
        for (let run = 1; run <= RUNS_PER_FEATURE; run++) {
          ...
          tasks.push(async () => {
            try {
              const promptText = buildDexifyPrompt(f, b);
              ...
              await insertDexifyFeatureResponse({
                brand_name:      b,
              ...
              console.error(`[dexify-feature-collection] error: ${brandName}/${feature.feature_id}/run${run}:`, err);
              if (!pairResults.has(pairKey)) pairResults.set(pairKey, []);
    ...
    console.log(`[dexify-feature-collection] start — model=${model}, date=${today}, brands=${brands.length}, tasks=${expected}`);
    const results   = await runWithConcurrency(tasks, BATCH_CONCURRENCY, BATCH_DELAY_MS);
    const succeeded = results.filter((r) => r.success).length;
    const failed    = expected - succeeded;
    console.log(`[dexify-feature-collection] done — succeeded=${succeeded}/${expected}, failed=${failed}`);
        ...
        groundingTasks.push(async () => {
          try {
            ...
            await insertDexifyFeatureResponse({
              ...
              has_capability, evidence, limitations, confidence, key_terms, terminology_tags,
              raw_json: parseError ? { raw: rawText.slice(0, 2000) } : parsed,
              parse_error: parseError, grounded: true,
            });
            groundingRan++;
          } catch (err) {
            console.error(`[dexify-feature-collection] grounding error: ${b}/${f.feature_id}:`, err);
            groundingFailed++;
          }
        });
      }
      if (groundingTasks.length > 0) {
        console.log(`[dexify-feature-collection] grounding pass: ${groundingTasks.length} pairs`);
    ...
    if (failed > 0) {
      ...
      }).catch((e) => console.error("[alert] dexify feature collection email failed:", e));
    }

    return Response.json({
      mode: "dexify_feature_collection", model, date: today,
      brands: brands.length, features: DEXIFY_FEATURES.length,
