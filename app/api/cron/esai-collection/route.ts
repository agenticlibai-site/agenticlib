...
import { ESAI_PROMPTS, ESAI_COLLECTION_SYSTEM_PROMPT } from "@/lib/brand-visibility/esai-prompts";
import { initEsaiDB, insertEsaiRawResponse } from "@/lib/brand-visibility/db";
...
// ── Split-job design ───────────────────────────────────────────────────────────
//   ?model=claude-haiku-4-5  →  07:05 UTC daily
//   ?model=gpt-4o-mini       →  07:15 UTC daily
// esai-aggregate runs at 07:50 UTC.
...
async function callClaude(promptText: string): Promise<ModelResult> {
  const res = await anthropic.messages.create({
    ...
    system: ESAI_COLLECTION_SYSTEM_PROMPT,
...
async function callGPT(promptText: string): Promise<ModelResult> {
  const res = await openai.chat.completions.create({
    ...
    messages: [
      { role: "system", content: ESAI_COLLECTION_SYSTEM_PROMPT },
...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const prompt of ESAI_PROMPTS) {
    ...
    console.log(`[esai-collection] start — model=${model}, date=${today}, prompts=${ESAI_PROMPTS.length}, tasks=${expected}`);
    const results   = await runWithConcurrency(tasks, BATCH_CONCURRENCY, BATCH_DELAY_MS);
    const succeeded = results.filter((r) => r.success).length;
    const failed    = expected - succeeded;
    console.log(`[esai-collection] done — succeeded=${succeeded}/${expected}, failed=${failed}`);

    if (failed > 0) {
      await sendEmail({
        subject: `[FeatureStream] ALERT — ESAI Collection failed (${model}, ${today})`,
        html: `<h2>ESAI Brand Coverage — Collection Failures</h2><p>Model: ${model} | Date: ${today} | Succeeded: ${succeeded}/${expected} | Failed: ${failed}</p>`,
      }).catch(() => {});
    }

    return Response.json({ mode: "esai_collection", model, date: today, prompts: ESAI_PROMPTS.length, expected, succeeded, failed });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`[cron] esai-collection crashed (${model}):`, message);
