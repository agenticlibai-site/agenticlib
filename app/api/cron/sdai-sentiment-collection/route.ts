...
import {
  SDAI_SENTIMENT_CLUSTERS, SDAI_SENTIMENT_SYSTEM_PROMPT, buildSdaiSentimentPrompt,
} from "@/lib/brand-visibility/sdai-sentiment";
import { initSdaiDB, getSdaiTopBrandsForFeatureScoring, insertSdaiSentimentResponse } from "@/lib/brand-visibility/db";
import { sendEmail } from "@/lib/email";

export const dynamic     = "force-dynamic";
export const maxDuration = 300;

// ?model=claude-haiku-4-5  →  09:00 UTC daily
// ?model=gpt-4o-mini       →  09:30 UTC daily
// 12 brands × 2 prompts = 24 calls per model per day.
// Weekly aggregation: sdai-sentiment-aggregate, Sunday 03:00 UTC.
...
async function callClaude(promptText: string): Promise<string> {
  const res = await anthropic.messages.create({
    ...
    system: SDAI_SENTIMENT_SYSTEM_PROMPT,
...
async function callGPT(promptText: string): Promise<string> {
  const res = await openai.chat.completions.create({
    ...
    messages: [
      { role: "system", content: SDAI_SENTIMENT_SYSTEM_PROMPT },
...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (brands.length === 0) {
      return Response.json({ mode: "sdai_sentiment_collection", date: today, note: "no brands in LOCKED_SDAI_BRANDS yet" });
    }

    const tasks: (() => Promise<{ success: boolean }>)[] = [];
    for (const brandName of brands) {
      for (const cluster of SDAI_SENTIMENT_CLUSTERS) {
        const b = brandName; const c = cluster;
        tasks.push(async () => {
          try {
            const promptText = buildSdaiSentimentPrompt(b, c.bucket_tag);
            ...
            await insertSdaiSentimentResponse({
            ...
            console.error(`[sdai-sentiment-collection] error: ${brandName}/${cluster.bucket_tag}:`, err);
    ...
    console.log(`[sdai-sentiment-collection] start — model=${model}, date=${today}, brands=${brands.length}, tasks=${expected}`);
    const results   = await runWithConcurrency(tasks, BATCH_CONCURRENCY, BATCH_DELAY_MS);
    const succeeded = results.filter((r) => r.success).length;
    const failed    = expected - succeeded;
    console.log(`[sdai-sentiment-collection] done — succeeded=${succeeded}/${expected}, failed=${failed}`);

    if (failed > 0) {
      await sendEmail({
        subject: `[FeatureStream] ALERT — SDAI Sentiment Collection failed (${model}, ${today})`,
        html: `<h2>SDAI Sentiment — Failures</h2><p>Model: ${model} | Date: ${today} | Succeeded: ${succeeded}/${expected} | Failed: ${failed}</p>`,
      }).catch(() => {});
    }

    return Response.json({ mode: "sdai_sentiment_collection", model, date: today, brands: brands.length, expected, succeeded, failed });
  } catch (err) {
    const message = err instanceof Error ? err.message : String(err);
    console.error(`[cron] sdai-sentiment-collection crashed (${model}):`, message);
