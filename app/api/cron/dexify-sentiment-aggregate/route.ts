import {
  initDexifyDB,
  getDexifySentimentResponsesForWeek,
  getPrevDexifySentimentScores,
  upsertDexifySentimentScore,
  upsertDexifySentimentDrift,
...
// Runs Sunday 2:00 AM UTC (0 2 * * 0) — after a full Mon–Sat collection week.
// 1. Aggregates the past 7 days of dexify_sentiment_responses into dexify_sentiment_scores.
// 2. Compares to previous week and writes drift flags to dexify_sentiment_drift.
...
export async function GET(request: Request) {
  ...
  try {
    ...
    const responses = await getDexifySentimentResponsesForWeek(weekStart, weekEnd);

    if (responses.length === 0) {
      return Response.json({
        mode:       "dexify_sentiment_aggregate",
        week_start: weekStart,
        week_end:   weekEnd,
        aggregated: 0,
        note:       "no dexify_sentiment_responses found for this week — collection may not have run yet",
      });
    }

    // Reuse the same aggregation + drift logic as the sales pipeline
    const aggregated = aggregateResponses(responses, weekStart);
    for (const row of aggregated) await upsertDexifySentimentScore(row);

    const previousScores = await getPrevDexifySentimentScores(prevWeekStart);
    const driftResults   = computeDrift(aggregated, previousScores);
    for (const row of driftResults) await upsertDexifySentimentDrift(row);
    ...
    if (driftFlags.length > 0) {
      ...
      }).catch((e) => console.error("[alert] dexify sentiment drift email failed:", e));
    }
