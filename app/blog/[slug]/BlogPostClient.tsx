...
export default function BlogPostClient({ blog, related }: Props) {
  useEffect(() => {
    trackEvent("blog_post_viewed", { blog_slug: blog.slug, blog_title: blog.title });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {related.map((rel) => (
                <Link
                  key={rel.slug}
                  href={`/blog/${rel.slug}`}
                  onClick={() => trackEvent("related_blog_clicked", { blog_slug: rel.slug })}
                  className="group glass-card rounded-2xl overflow-hidden flex gap-4 p-3
