...
const SALT = "|esai_gate_agenticlib_2026";
...
async function verifyPassword(formData: FormData) {
  ...
  const correct = process.env.ESAI_ACCESS_PASSWORD ?? "";

  if (!entered || entered !== correct) {
    redirect("/solutions/construction?error=1");
  }

  const token = computeToken(entered);
  const jar = await cookies();
  jar.set("esai_auth", token, {
    httpOnly: true,
    secure: true,
    sameSite: "lax",
    path: "/",
  ...
  redirect("/product/esai-visibility");
}
...
export default async function ConstructionAccessPage({
  searchParams,
}: {
    ...
    >
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 18, width: "100%", maxWidth: 400 }}>
        ...
        >
          ...
          >
            Brand Intelligence · Construction AI
          </div>
          ...
          <form action={verifyPassword} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <style>{`
              ...
              .esai-submit:hover { background: #DC2D0B; }
              .esai-submit:active {
            ...
            `}</style>
            <button type="submit" className="esai-submit">
              Access report
            </button>
          </form>
        </div>
