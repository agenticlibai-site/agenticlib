...
export async function getDexifySentimentData(): Promise<{
  rows: {
    brand_name:      string;
    bucket_tag:      string;
    positive_count:  number;
...
let _ralfiDbInit: Promise<void> | null = null;

export function initRalfiDB(): Promise<void> {
  if (!_ralfiDbInit) {
    _ralfiDbInit = _runRalfiDDL().catch(err => {
      _ralfiDbInit = null;
      return Promise.reject(err);
    });
  }
...
async function _runRalfiDDL(): Promise<void> {
  await sql`
    CREATE TABLE IF NOT EXISTS ralfi_raw_responses (
      id             SERIAL PRIMARY KEY,
      date           DATE    NOT NULL,
      prompt_id      INTEGER NOT NULL,
      prompt_text    TEXT    NOT NULL,
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS ralfi_daily_summary (
      id            SERIAL PRIMARY KEY,
      date          DATE    NOT NULL,
      brand         TEXT    NOT NULL,
      model         TEXT    NOT NULL,
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS ralfi_denylist (
      brand_name TEXT NOT NULL UNIQUE,
      reason     TEXT,
      created_at TIMESTAMP DEFAULT NOW()
    )
  ...
  `;
  await sql`
    CREATE UNIQUE INDEX IF NOT EXISTS ralfi_sentiment_responses_unique
    ON ralfi_sentiment_responses (brand_name, prompt_id, model, run_date)
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS ralfi_sentiment_scores (
      id              SERIAL PRIMARY KEY,
      brand_name      TEXT    NOT NULL,
      bucket_tag      TEXT    NOT NULL,
      positive_count  INTEGER NOT NULL DEFAULT 0,
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS ralfi_sentiment_drift (
      id           SERIAL PRIMARY KEY,
      brand_name   TEXT    NOT NULL,
      bucket_tag   TEXT    NOT NULL,
      week_start   DATE    NOT NULL,
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS ralfi_feature_responses (
      id               SERIAL PRIMARY KEY,
      brand_name       TEXT    NOT NULL,
      feature_id       TEXT    NOT NULL,
      feature_tag      TEXT    NOT NULL,
  ...
  `;
  await sql`
    CREATE UNIQUE INDEX IF NOT EXISTS ralfi_feature_responses_unique
    ON ralfi_feature_responses (brand_name, feature_id, model, run_number, run_date)
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS ralfi_feature_scores (
      id                 SERIAL PRIMARY KEY,
      brand_name         TEXT    NOT NULL,
      feature_id         TEXT    NOT NULL,
      feature_tag        TEXT    NOT NULL,
...
export async function insertRalfiRawResponse(row: {
  date:          string;
  promptId:      number;
  promptText:    string;
  clusterTag:    string;
...
export interface RalfiTopBrandRow {
  brand:          string;
  total_mentions: number;
  avg_position:   number | null;
}
export interface RalfiClusterRow {
  cluster_tag:    string;
  brand:          string;
  total_mentions: number;
  avg_position:   number | null;
}
export interface RalfiTrendRow {
  date:          string;
  brand:         string;
  mention_count: number;
}

export async function getRalfiTopBrands(limit = 25): Promise<RalfiTopBrandRow[]> {
  await initRalfiDB();
  const result = await sql`
    SELECT brand, SUM(mention_count)::int AS total_mentions, AVG(avg_position)::float AS avg_position
    FROM ralfi_daily_summary
    WHERE LOWER(brand) NOT IN (SELECT LOWER(brand_name) FROM ralfi_denylist)
    GROUP BY brand ORDER BY total_mentions DESC LIMIT ${limit}
  `;
  return result.rows as RalfiTopBrandRow[];
}

export async function getRalfiByCluster(): Promise<RalfiClusterRow[]> {
  await initRalfiDB();
  const result = await sql`
    SELECT cluster_tag, brand, SUM(mention_count)::int AS total_mentions, AVG(avg_position)::float AS avg_position
    FROM ralfi_daily_summary
    WHERE LOWER(brand) NOT IN (SELECT LOWER(brand_name) FROM ralfi_denylist)
    GROUP BY cluster_tag, brand ORDER BY cluster_tag, total_mentions DESC
  `;
  return result.rows as RalfiClusterRow[];
}

export async function getRalfiTrend(days = 7): Promise<RalfiTrendRow[]> {
  await initRalfiDB();
  const result = await sql`
    ...
    GROUP BY date, brand ORDER BY date, mention_count DESC
  `;
  return result.rows as RalfiTrendRow[];
}

// ── Ralfi feature pipeline ────────────────────────────────────────────────────

export async function getRalfiTopBrandsForFeatureScoring(): Promise<string[]> {
  const { LOCKED_RALFI_BRANDS } = await import("@/lib/brand-visibility/ralfi-features");
  return [...LOCKED_RALFI_BRANDS];
}

export async function insertRalfiFeatureResponse(row: {
  brand_name:       string;
  feature_id:       string;
  feature_tag:      string;
  model:            string;
  ...
  const grounded = row.grounded ?? false;
  await sql`
    INSERT INTO ralfi_feature_responses
      (brand_name, feature_id, feature_tag, model, run_number, run_date,
...
export async function getRalfiFeatureResponsesForScoring(runDate: string): Promise<{
  brand_name: string; feature_id: string; feature_tag: string; model: string;
  has_capability: string | null; evidence: string | null; confidence: string | null;
  parse_error: boolean; grounded: boolean;
}[]> {
...
export async function upsertRalfiFeatureScore(row: {
  brand_name: string; feature_id: string; feature_tag: string;
  ...
  const grounded_source = row.grounded_source ?? false;
  await sql`
    INSERT INTO ralfi_feature_scores
      (brand_name, feature_id, feature_tag, score, score_band,
...
// ── Ralfi sentiment pipeline ──────────────────────────────────────────────────

export async function insertRalfiSentimentResponse(row: {
  ...
  const limitationsJson = JSON.stringify(row.limitations ?? []);
  await sql`
    INSERT INTO ralfi_sentiment_responses
      (brand_name, prompt_id, bucket_tag, model, run_date,
       sentiment, confidence, descriptors, limitations, raw_json, parse_error)
    VALUES
      (${row.brand_name}, ${row.prompt_id}, ${row.bucket_tag}, ${row.model},
...
export async function getRalfiSentimentResponsesForWeek(weekStart: string, weekEnd: string): Promise<{
  brand_name: string; bucket_tag: string; sentiment: string | null;
  confidence: string | null; descriptors: string[] | null; parse_error: boolean;
}[]> {
  const result = await sql`
...
export async function getPrevRalfiSentimentScores(prevWeekStart: string): Promise<{
  brand_name: string; bucket_tag: string;
  positive_count: number; neutral_count: number; negative_count: number; total_count: number;
}[]> {
  const result = await sql`
    SELECT brand_name, bucket_tag, positive_count, neutral_count, negative_count, total_count
    FROM ralfi_sentiment_scores WHERE week_start = ${prevWeekStart}::date
  `;
  return result.rows as {
    brand_name: string; bucket_tag: string;
    positive_count: number; neutral_count: number; negative_count: number; total_count: number;
...
export async function upsertRalfiSentimentScore(row: {
  brand_name: string; bucket_tag: string;
  positive_count: number; neutral_count: number; negative_count: number; total_count: number;
  ...
  const uniqueFlagJson = JSON.stringify(row.unique_flags);
  await sql`
    INSERT INTO ralfi_sentiment_scores
      (brand_name, bucket_tag, positive_count, neutral_count, negative_count,
       total_count, top_descriptors, unique_flags, week_start)
    VALUES
      (${row.brand_name}, ${row.bucket_tag}, ${row.positive_count},
...
export async function upsertRalfiSentimentDrift(row: {
  ...
  await sql`
    INSERT INTO ralfi_sentiment_drift
...
let _hsaiDbInit: Promise<void> | null = null;

export function initHsaiDB(): Promise<void> {
  if (!_hsaiDbInit) {
    _hsaiDbInit = _runHsaiDDL().catch(err => {
      _hsaiDbInit = null;
      return Promise.reject(err);
    });
  }
...
async function _runHsaiDDL(): Promise<void> {
  await sql`
    CREATE TABLE IF NOT EXISTS hsai_raw_responses (
      id             SERIAL PRIMARY KEY,
      date           DATE    NOT NULL,
      prompt_id      INTEGER NOT NULL,
      prompt_text    TEXT    NOT NULL,
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS hsai_daily_summary (
      id            SERIAL PRIMARY KEY,
      date          DATE    NOT NULL,
      brand         TEXT    NOT NULL,
      model         TEXT    NOT NULL,
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS hsai_feature_responses (
      id               SERIAL PRIMARY KEY,
      brand_name       TEXT    NOT NULL,
      feature_id       TEXT    NOT NULL,
      feature_tag      TEXT    NOT NULL,
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS hsai_feature_scores (
      id                 SERIAL PRIMARY KEY,
      brand_name         TEXT    NOT NULL,
      feature_id         TEXT    NOT NULL,
      feature_tag        TEXT    NOT NULL,
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS hsai_sentiment_responses (
      id          SERIAL PRIMARY KEY,
      brand_name  TEXT    NOT NULL,
      prompt_id   INTEGER NOT NULL,
      bucket_tag  TEXT    NOT NULL,
  ...
  `;
  await sql`
    CREATE TABLE IF NOT EXISTS hsai_sentiment_scores (
      id              SERIAL PRIMARY KEY,
      brand_name      TEXT    NOT NULL,
      bucket_tag      TEXT    NOT NULL,
      positive_count  INTEGER NOT NULL DEFAULT 0,
...
export async function insertHsaiRawResponse(row: {
  date: string; promptId: number; promptText: string; clusterTag: string;
  model: string; modelSnapshot: string; runNumber: number; brands: string[];
}): Promise<void> {
  await sql`
    INSERT INTO hsai_raw_responses
      (date, prompt_id, prompt_text, cluster_tag, model, model_snapshot, run_number, brands)
    VALUES
      (${row.date}::date, ${row.promptId}, ${row.promptText}, ${row.clusterTag},
       ${row.model}, ${row.modelSnapshot}, ${row.runNumber}, ${JSON.stringify(row.brands)}::jsonb)
...
export async function getHsaiBrandsForFeatureScoring(): Promise<string[]> {
  const { LOCKED_HSAI_BRANDS } = await import("@/lib/brand-visibility/hsai-features");
  return [...LOCKED_HSAI_BRANDS];
}

export async function insertHsaiFeatureResponse(row: {
  brand_name: string; feature_id: string; feature_tag: string;
  model: string; run_number: number; run_date: string;
  has_capability: string | null; evidence: string | null;
  limitations: string | null; confidence: string | null;
  ...
  await sql`
    INSERT INTO hsai_feature_responses
      (brand_name, feature_id, feature_tag, model, run_number, run_date,
       has_capability, evidence, limitations, confidence, terminology_tags,
       raw_json, parse_error, grounded)
    VALUES
...
export async function getHsaiFeatureResponsesForScoring(runDate: string): Promise<{
  brand_name: string; feature_id: string; feature_tag: string;
  has_capability: string | null; confidence: string | null;
  evidence: string | null; parse_error: boolean; grounded: boolean;
}[]> {
...
export async function upsertHsaiFeatureScore(row: {
  brand_name: string; feature_id: string; feature_tag: string;
  score: number | null; score_band: string;
  runs_agreeing: number | null; runs_total: number;
  flagged_for_review: boolean; flag_reason: string | null;
  ...
  await sql`
    INSERT INTO hsai_feature_scores
      (brand_name, feature_id, feature_tag, score, score_band,
       runs_agreeing, runs_total, flagged_for_review, flag_reason, notes, grounded_source)
    VALUES
      (${row.brand_name}, ${row.feature_id}, ${row.feature_tag},
...
export async function insertHsaiSentimentResponse(row: {
  brand_name: string; prompt_id: number; bucket_tag: string; model: string; run_date: string;
  sentiment: string | null; confidence: string | null; descriptors: string[] | null;
  limitations: string | null; raw_json: object | null; parse_error: boolean;
}): Promise<void> {
  await sql`
    INSERT INTO hsai_sentiment_responses
      (brand_name, prompt_id, bucket_tag, model, run_date,
       sentiment, confidence, descriptors, limitations, raw_json, parse_error)
    VALUES
      (${row.brand_name}, ${row.prompt_id}, ${row.bucket_tag}, ${row.model}, ${row.run_date}::date,
...
export async function getHsaiSentimentResponsesForWeek(weekStart: string, weekEnd: string): Promise<{
  brand_name: string; bucket_tag: string; sentiment: string | null;
  confidence: string | null; descriptors: string[] | null; limitations: string | null;
}[]> {
  const result = await sql`
    SELECT brand_name, bucket_tag, sentiment, confidence, descriptors, limitations
    FROM hsai_sentiment_responses
    WHERE run_date >= ${weekStart}::date AND run_date <= ${weekEnd}::date
      AND parse_error = false
    ORDER BY brand_name, bucket_tag
  `;
...
export async function upsertHsaiSentimentScore(row: {
  brand_name: string; bucket_tag: string; week_start: string;
  positive_count: number; neutral_count: number; negative_count: number; total_count: number;
  top_descriptors: string[];
}): Promise<void> {
  await sql`
    INSERT INTO hsai_sentiment_scores
      (brand_name, bucket_tag, week_start, positive_count, neutral_count, negative_count,
       total_count, top_descriptors)
    VALUES
      (${row.brand_name}, ${row.bucket_tag}, ${row.week_start}::date,
       ${row.positive_count}, ${row.neutral_count}, ${row.negative_count}, ${row.total_count},
       ${row.top_descriptors}::text[])
    ON CONFLICT (brand_name, bucket_tag, week_start) DO UPDATE SET
      positive_count = EXCLUDED.positive_count, neutral_count = EXCLUDED.neutral_count,
      negative_count = EXCLUDED.negative_count, total_count = EXCLUDED.total_count,
      top_descriptors = EXCLUDED.top_descriptors, scored_at = NOW()
  `;
}
